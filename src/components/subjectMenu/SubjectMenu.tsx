...
import {ActionType as ObserverActionType, IDictionary, IObserver, IInitialState as IObserverInitialState} from '../../store/observer-reducer'
...
function getNonRegisteredObserversForSubject(
  ...
  function getSubjectObserverMenuItems(menuType:MenuType, subjectId:string, allObservers:IDictionary<IObserver>, subjectObservers:IDictionary<IObserver>): IMenuItem[] {
    let menuItems: IMenuItem[] = [];
    console.log(menuType);
    
    switch (menuType) {
      case MenuType.SubjectRegisterObserverMenu: {
        const nonRegisteredObservers = Object.values(getNonRegisteredObserversForSubject(allObservers,subjectObservers));
  ...
  const SubjectMenu: React.FC<IProps> = props => {
        ...
        switch (menuType) {
          ...
          case MenuType.SubjectRegisterObserverMenu:
          case MenuType.SubjectUnregisterObserverMenu:
            menuItems = getSubjectObserverMenuItems(menuType,subjectId,allObservers,subjectObservers);
            break;
  ...
  return (
    <MenuStyles className='menu'>
      ...
      <MenuItemsList menuItems={menuItems}></MenuItemsList>
    </MenuStyles>
  )
};

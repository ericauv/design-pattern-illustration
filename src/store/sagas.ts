...
function* notifyObservers(action:IDispatchAction) {
    // Notify observers
    try{
        yield put({type:ActionType.NOTIFY_OBSERVERS, payload:{subjectId:action.payload.subjectId}});
        yield delay(500);
        yield put({type:ActionType.UNNOTIFY_OBSERVERS, payload:{subjectId:action.payload.subjectId}});
    }
    catch (e) {
